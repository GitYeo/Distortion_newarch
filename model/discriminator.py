...
class Discriminator(nn.Module):
    ...
    def __init__(self, FLAGS):
        ...
        layers = [ConvBNRelu(3, FLAGS.discriminator_channels,spectral=True)]
        for _ in range(FLAGS.discriminator_blocks-1):
            layers.append(ConvBNRelu(FLAGS.discriminator_channels, FLAGS.discriminator_channels,spectral=True))

        layers.append(nn.AdaptiveAvgPool2d(output_size=(1, 1)))
        self.before_linear = nn.Sequential(*layers)
        self.linear = spectral_norm(nn.Linear(FLAGS.discriminator_channels, 1))
        self.initialize()
