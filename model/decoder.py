...
class Decoder(nn.Module):
    ...
    def __init__(self, FLAGS):
        ...
        for _ in range(FLAGS.decoder_blocks - 4):
        ...
        #layers.append(ConvBNRelu(self.channels, self.channels,stride=2))
        layers.append(ConvBNRelu(self.channels, FLAGS.redundant_length,stride=2))
        layers.append(nn.AdaptiveAvgPool2d(output_size=(1, 1)))
        self.layers = nn.Sequential(*layers)

        self.linear = nn.Linear(FLAGS.redundant_length, FLAGS.redundant_length)

    def forward(self, image_with_wm, encoded_flag=True):
        if encoded_flag is True:
