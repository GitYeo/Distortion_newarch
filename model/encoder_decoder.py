...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, image, message, generator, necst, identity=False, noiser=None):
        ...
        if identity is False:
            ...
            adv_redundant_decoded_message = self.decoder(adv_image,encoded_flag=False)
            redundant_decoded_message = self.decoder(encoded_image, encoded_flag=True)
            ...
            if noiser is not None:
                ...
                decoded_message = self.decoder(noised_image,encoded_flag=False)
            else:
                decoded_message = self.decoder(encoded_image,encoded_flag=True)
